...
\vspace{-0.2cm}
\begin{itemize}
	\item In the following we will \hl{focus on stochastic policies} $\pi(\bm{u} \vert \bm{x})$ .\pause
...
\Input{diff. stochastic policy fct. $\pi(\bm{u} \vert \bm{x},\bm{\theta})$ and value fct. $\hat{v}(\bm{x},\bm{w})$}
\Param{step sizes $\{\alpha_{w}, \alpha_{\theta}\}\in\left\{\Re \vert 0<\alpha\right\}$}
\Init{weights $\bm{w}\in\Re^{\zeta}$ and $\bm{\theta}\in\Re^d$ arbitrarily, memory $\bm{\mathcal{D}}$}\pause
 \For{$j=1,2,\ldots,$ (sub-)episodes}{
		...
		collect a set of tuples $\left\langle \bm{x}_k, \bm{u}_k, r_{k+1}, \bm{x}_{k+1}\right\rangle$ by a rollout using $\pi(\bm{u} \vert \bm{x},\bm{\theta}_j)$\;\pause
