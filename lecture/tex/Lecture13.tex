...
\begin{itemize}
	...
	\item Using the advantage $a_\pi(\bm{x},\bm{u})= q_{\pi}(\bm{x},\bm{u})-v_{\pi}(\bm{x})$ we can calculate the performance of an updated policy $\bm{\pi}\rightarrow\tilde{\bm{\pi}}$\footnote{proof from: S. Kakade and J. Langford, \textit{Approximately optimal approximate reinforcement learning}, ICML \textbf{2}:267-274, 2002}:
		\begin{equation}
...
\begin{itemize}
	...
	\begin{split}
		 ...
		 \text{s.t.} \quad\quad  & \overline{D}_{\mathrm{KL}}(\bm{\theta}_k, \bm{\theta})\leq\kappa
	\end{split}
	\end{equation}\pause
	with 
\end{itemize}
...
\begin{figure}
\includegraphics[height=5.75cm]{fig/lec13/TRPO_Style_Updates}
...
\vspace{0.15cm}
	...
	\begin{split}
		 ...
		 \text{s.t.} \quad\quad  & \El{D_{\mathrm{KL}}(\pi_{\bm{\theta}_k}(\cdot|\bm{X}) \parallel \pi_{\bm{\theta}}(\cdot|\bm{X}))}{{\pi_{\bm{\theta}_k}}}\leq\kappa.
	\end{split}
	\end{equation}
	\vspace{-0.25cm}
\begin{itemize}\pause
...
\begin{figure}
\includegraphics[height=5.25cm]{fig/lec13/Algo_Compare}
