...
\frame{\frametitle{General TD prediction updates}
  ...
  \end{equation}
  \begin{itemize}
  \item $\alpha\in\left\{\Re\:\vert\:0<\alpha<1\right\}$ is the forgetting factor / step size.
  \item $g_k$ is the \hl{target} of the incremental update rule.
  \item To execute the update \eqref{eq:MC_non_stat_up_lec05} one has to wait until the episode's termination since only then is $g_k$ available (MC requirement).
  \end{itemize}\pause
...
\frame{\frametitle{TD error and its relation to the MC error }
  \onslide<1->{Let's assume that the TD(0) estimate $\hat{v}(x)$ is not changing over one episode as it would be for MC prediction:
    \begin{equation}
      \begin{split}
        \underbrace{g_k - \hat{v}(x_k)}_{\text{MC-error}} &= r_{k+1} + \gamma g_{k+1} - \hat{v}(x_k)	+\gamma  \hat{v}(x_{k+1})	-\gamma  \hat{v}(x_{k+1}),\\}
...
\frame{\frametitle{Overview of the RL methods considered so far}
  \begin{figure}
    ...
    \caption{Comparison of the RL methods considered so far with regard to the update rules (source: R. Sutton and G. Barto, \emph{Reinforcement Learning: An Introduction}, 2018, \href{https://creativecommons.org/licenses/by-nc-nd/2.0/}{CC BY-NC-ND 2.0})}
...
\frame{\frametitle{Driving home example}
  \begin{figure}
    ...
    \caption{Updates by MC (left) and TD (right) for $\alpha = 1$ (source: R. Sutton and G. Barto, \emph{Reinforcement Learning: An Introduction}, 2018, \href{https://creativecommons.org/licenses/by-nc-nd/2.0/}{CC BY-NC-ND 2.0})}
...
\frame{\frametitle{Batch training: AB-example (1)}
  \begin{columns}[t,onlytextwidth]
	...
	\begin{figure}
	  ...
	  \caption{Example environment (source: R. Sutton and G. Barto, \emph{Reinforcement Learning: An Introduction}, 2018, \href{https://creativecommons.org/licenses/by-nc-nd/2.0/}{CC BY-NC-ND 2.0})}
...
\frame{\frametitle{Batch training: AB-example (2)}
  ...
  \begin{align*}
    \text{MC}: \quad &\hat{v}(x_k) \leftarrow \hat{v}(x_k) + \alpha\left[g_k - \hat{v}(x_k)\right],\\
    \text{TD}:	\quad&\hat{v}(x_k) \leftarrow \hat{v}(x_k) + \alpha\left[r_{k+1}+\gamma\hat{v}(x_{k+1}) - \hat{v}(x_k)\right] .
  \end{align*}\pause
  Then, in steady state one receives:
  \begin{align*}
    \text{MC}: \quad &0=\alpha\left[g_k - \hat{v}(x_k)\right]=g_k - \hat{v}(x_k),\\
    \text{TD}:	\quad&0=\alpha\left[r_{k+1}+\gamma\hat{v}(x_{k+1}) - \hat{v}(x_k)\right] =r_{k+1}+\gamma\hat{v}(x_{k+1}) - \hat{v}(x_k).
  \end{align*}\pause
  Considering a batch learning sweep over $j=1,\ldots,J$ episodes:
  \begin{align*}
    \text{MC}: \quad &0=\sum_{j=1}^J g_{k,j} - \hat{v}(x_{k,j}),\\
    \text{TD}:	\quad&0=\sum_{j=1}^J r_{k+1,j}+\gamma\hat{v}(x_{k+1,j}) - \hat{v}(x_{k,j}).
  \end{align*}
}
...
\frame{\frametitle{Batch training: AB-example (3)}
  ...
  \begin{alignat*}{3}
    \text{MC}|_{x=\text{B}}: \quad &0=\sum_{j=1}^J g_{k,j} - \hat{v}(x_{k,j})\quad &&\Leftrightarrow \quad &&\hat{v}(\text{B})=\frac{1}{J}\sum_{j=1}^J g_{k,j},\\
    \text{TD}|_{x=\text{B}}:	\quad&0=\sum_{j=1}^J r_{k+1,j}- \hat{v}(x_{k,j})\quad &&\Leftrightarrow \quad &&\hat{v}(\text{B})=\frac{1}{J}\sum_{j=1}^J g_{k,j}.
  \end{alignat*}\pause
  This is the average return of the available episodes from \tabref{tab:state_reward_AB_example} , i.e., $6\times 1$ and $2\times 0$:
  \begin{equation}
    \hat{v}(\text{B})|_{\text{MC}}=\hat{v}(\text{B})|_{\text{TD}}=\frac{6}{8}=0.75\, .
  \end{equation}
}
...
\frame{\frametitle{Batch training: AB-example (4)}
  ...
  \end{itemize}\pause
  \begin{align*}
    \text{MC}: \quad &0=\sum_{j=1}^J g_{k,j} - \hat{v}(x_{k,j})=\sum_{j=1}^J g_{k,j} - \hat{v}(A),\\
    \text{TD}:	\quad&0=\sum_{j=1}^J r_{k+1,j}+\gamma\hat{v}(x_{k+1,j}) - \hat{v}(x_{k,j})=\sum_{j=1}^J \gamma\hat{v}(B) - \hat{v}(A).
  \end{align*}\pause
  Looking at \tabref{tab:state_reward_AB_example} there is only one episode visiting state A, where the sample return is $g_{k,j}=0$. Hence it follows that:
  \begin{equation*}
    \hat{v}(\text{A})|_{\text{MC}} =0, \quad \hat{v}(\text{A})|_{\text{TD}} =\gamma\hat{v}(B)=\frac{3}{4}.
  \end{equation*}
...
\frame{\frametitle{TD-based on-policy control (SARSA)}
  ...
  \begin{algorithm}[H]
    ...
    \SetKwInput{Init}{init}
    \SetKwInput{Param}{parameter}
    \Param{$\varepsilon\in\left\{\Re|0<\varepsilon\ll 1\right\}, \quad \alpha\in\left\{\Re|0<\alpha<1\right\}$}
...
\frame{\frametitle{SARSA example: forest tree MDP (1)}
  \begin{figure}
    \includegraphics[height=0.65\textheight]{fig/lec05/Forest_Tree_Sarsa_alpha_0.2.png}
    \caption{SARSA-based control with \hl{$\alpha_{\text{SARSA}}=0.2$} and $\varepsilon$-greedy policy with $\varepsilon=0.2$ of forest tree MDP over the number of episodes being evaluated (mean and standard deviation are calculated based on 2000 independent runs)}
...
\frame{\frametitle{SARSA example: forest tree MDP (2)}
  \begin{figure}
    \includegraphics[height=0.65\textheight]{fig/lec05/Forest_Tree_Sarsa_alpha_0.1.png}
    \caption{SARSA-based control with \hl{$\alpha_{\text{SARSA}}=0.1$} and $\varepsilon$-greedy policy with $\varepsilon=0.2$ of forest tree MDP over the number of episodes being evaluated (mean and standard deviation are calculated based on 2000 independent runs)}
...
\frame{\frametitle{SARSA example: forest tree MDP (3)}
  \begin{figure}
    \includegraphics[height=0.65\textheight]{fig/lec05/Forest_Tree_Sarsa_alpha_adapt}
    \caption{SARSA-based control with \hl{adaptive $\alpha_{\text{SARSA}}={1}/{\sqrt{j}}$} ($j=\text{episode}$) and $\varepsilon$-greedy policy with $\varepsilon=0.2$ of forest tree MDP over the number of episodes being evaluated (mean and standard deviation are calculated based on 2000 independent runs)}
...
\frame{\frametitle{$Q$-learning control example: cliff walking}
  \begin{columns}[c,onlytextwidth]
	...
	\begin{figure}
	  ...
	  \caption{Cliff walking environment (source: R. Sutton and G. Barto, \emph{Reinforcement Learning: An Introduction}, 2018, \href{https://creativecommons.org/licenses/by-nc-nd/2.0/}{CC BY-NC-ND 2.0})}
