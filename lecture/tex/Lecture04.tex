...
\frame{\frametitle{Monte Carlo methods vs. dynamic programming}
  ...
  \item In general: broad class of computational algorithms relying on \hl{repeated random sampling} to obtain numerical results
    \pause
  \item \hl{Warning:} It's easy to \href{https://young.physics.ucsc.edu/115/randu.pdf}{mess up} your random number generator with \href{https://acta.imeko.org/index.php/acta-imeko/article/view/IMEKO-ACTA-09\%20\%282020\%29-04-17}{dire consequences}.
  \end{itemize}
}
...
\frame{\frametitle{General Monte Carlo (MC) methods' characteristics}
  ...
  \onslide<1->
  \shortstack{
    \includegraphics[width=0.5\textwidth]{fig/lec04/Monte_Carlo_City.jpg}\\
    Monte Carlo port \\
    (source: \href{https://www.flickr.com/photos/flynn_nrg/40890124713/}{www.flickr.com}, by \href{https://www.flickr.com/photos/flynn_nrg/}{Miguel Mendez} \href{https://creativecommons.org/licenses/by/2.0/}{CC BY 2.0})}
  \hfill
  \onslide<5->
  \shortstack{
    \includegraphics[width=0.3\textwidth]{fig/lec04/gambling-in-monte-carlo.jpg}\\
    Bond. James Bond.\\
    (Warner Bros.)}
}
...
\frame{\frametitle{Incremental implementation}
  ...
  \end{equation} \pause
  \begin{itemize}
  \item \hl{If a given decision problem is non-stationary}, using a forgetting factor $\alpha\in\left\{\Re\:\vert\: 0<\alpha<1\right\}$ allows for dynamic adaption:
  \end{itemize}
  \begin{equation}
...
\frame{\frametitle{Statistical properties of MC-based prediction (1)}
  \onslide<1->{First-time visit MC:
    ...
    \item The estimate's variance $\Var{\hat{v}_{\pi}}$ drops with $1/n$ ($n = \text{\#samples}$).
  \end{itemize}}
  \bigskip
  \onslide<2->{Every-time visit MC:
    ...
    \item One receives \hl{non-i.i.d.\ data} to estimate $\E{\hat{v}_{\pi}}$ and consequently this \hl{is biased} for any $n<\infty$. (Bug? Maybe it just has higher variance.)
    \item Only in the limit $n\rightarrow \infty$ one receives $\left(v_{\pi}(x)-\E{\hat{v}_{\pi}(x)}\right) \rightarrow 0$.
  \end{itemize}}
  \footnotesize
  \vspace{0.5cm}
  \onslide<1->More information: S. Singh and  R. Sutton, \emph{Reinforcement Learning with Replacing Eligibility Traces}, Machine Learning \textbf{22}:123--158, 1996
}
...
\frame{\frametitle{Applying Generalized Policy Iteration (GPI) to MC Control}
  \onslide<1->{GPI is directly applied to MC framework using action values:
    \begin{equation}
      \label{eq:GPI_MC}
      \pi_0 \rightarrow \hat{q}_{\pi_0} \rightarrow \pi_1 \rightarrow \hat{q}_{\pi_1} \rightarrow \cdots \rightarrow \pi^* \rightarrow \hat{q}_{\pi^*} \, .
  \end{equation}}
  \begin{itemize}
  ...
  \onslide<1->{\begin{figure}
      ...
      \begin{minipage}[c]{0.45\textwidth}
        \caption{Transferring GPI to MC-based control (source: R. Sutton and G. Barto, \emph{Reinforcement Learning: An Introduction}, 2018, \href{https://creativecommons.org/licenses/by-nc-nd/2.0/}{CC BY-NC-ND 2.0})}
	\label{fig:GPI_MC}
      \end{minipage}
  \end{figure}}
}
...
\frame{\frametitle{$\varepsilon$-greedy as an on-policy alternative}
  ...
  \item Exploration requirement:
    ...
    \item Visit all state-action pairs with probability
      \begin{equation}
	\pi(u\:\vert\: x) > 0 \qquad \forall\, \left\{x\in\mathcal{X}, u\in\mathcal{U}\right\}
        \hspace*{10em}
      \end{equation}
    \item<3-> Policies with this characteristic are called: \hl{soft}.
    \item<3-> The level of exploration can be tuned during learning.
    \end{itemize}
    \vfill
  \item<4-> \hl{$\varepsilon$-greedy on-policy learning}
    \begin{itemize}
    \item With probability $\varepsilon$ the agent's choice (i.e., the policy output) is replaced by a random action.
      \smallskip
    \item Probability of all non-greedy actions:
      \begin{equation}
        \varepsilon/\lvert\mathcal{U}\rvert
      \end{equation}
    \item Probability of the greedy action:
      \begin{equation}
        1-\varepsilon+\varepsilon/\lvert\mathcal{U}\rvert
      \end{equation}
      where $\lvert\mathcal{U}\rvert$ is the number of possible actions.
    \end{itemize}
  \end{itemize}
  ...
  \begin{tikzpicture}[remember picture,overlay]
    ...
    at (current page.north east) % Change position as needed
    {\rule{0em}{27ex}\includegraphics[width=0.3\textwidth]{fig/lec04/never-say-never-again-poster.jpg}\hspace*{3em}};
  \end{tikzpicture}
}
...
\frame{\frametitle{Algorithmic implementation $\varepsilon$-greedy MC-control}
  ...
  \begin{algorithm}[H]
    ...
    \SetKwInput{Init}{init}
    \SetKwInput{Param}{parameter}
    \Output{Optimal $\varepsilon$-greedy policy $\pi^*(u\:\vert\: x)$, \hspace{0.5cm}\textbf{parameter:} $\varepsilon\in\left\{\Re\:\vert\: 0<\varepsilon \ll 1\right\}$}%\Param{}
    \Init{$\pi_{i=0}(u\:\vert\: x)$ arbitrarily soft $\forall \, \left\{x\in\mathcal{X}, u\in\mathcal{U}\right\}$\newline
      $\hat{q}(x,u)$ arbitrarily $\forall \, \left\{x\in\mathcal{X}, u\in\mathcal{U}\right\}$\newline
	...
	\If{$\left\{x_{k}, u_{k}\right\}\notin \left\langle \left\{x_{0}, u_{0}\right\}, \ldots, \left\{x_{k-1}, u_{k-1}\right\}\right\rangle$}{
	  ...
	  $\pi_i(u\:\vert\: x_k)\leftarrow
          \begin{cases}
            1-\varepsilon+\varepsilon/\lvert\mathcal{U}\rvert & \text{when $u=\tilde{u}$}\\ \varepsilon/\lvert\mathcal{U}\rvert & \text{when $u\neq\tilde{u}$}
          \end{cases}
          $\;
	}
      }
...
\frame{\frametitle{$\varepsilon$-greedy policy improvement (1)}
  ...
  \begin{equation}
    \begin{split}
      q_{\pi}(x,\pi'(x)) &= \sum_{u}\pi'(u\:\vert\: x)\,q_{\pi}(x,u),\\
      &= \frac{\varepsilon}{\lvert\mathcal{U}\rvert }\sum_{u}q_{\pi}(x,u) + (1-\varepsilon)\max_{u}q_{\pi}(x,u),\\
      &\geq \frac{\varepsilon}{\lvert\mathcal{U}\rvert }\sum_{u}q_{\pi}(x,u) + (1-\varepsilon)\sum_{u}\frac{\pi(u\:\vert\: x)-\frac{\varepsilon}{\lvert\mathcal{U}\rvert }}{1-\varepsilon}q_{\pi}(x,u).
    \end{split}
  \end{equation}
  \normalsize
  In the inequality line, the second term is the weighted sum over action values given an $\varepsilon$-greedy policy. This weighted sum is bounded above by $\max_{u}q_{\pi}(x,u)$.
}
...
\frame{\frametitle{$\varepsilon$-greedy policy improvement (2)}
  ...
  \begin{equation}
    \begin{split}
      q_{\pi}(x,\pi'(x)) &\geq \frac{\varepsilon}{\lvert\mathcal{U}\rvert }\sum_{u}q_{\pi}(x,u) + (1-\varepsilon)\sum_{u}\frac{\pi(u\:\vert\: x)-\frac{\varepsilon}{\lvert\mathcal{U}\rvert }}{1-\varepsilon}q_{\pi}(x,u),\\
      &= \frac{\varepsilon}{\lvert\mathcal{U}\rvert }\sum_{u}\left(q_{\pi}(x,u) - q_{\pi}(x,u)\right) + \sum_{u}\pi(u\:\vert\: x)\,q_{\pi}(x,u),\\
      &=\sum_{u}\pi(u\:\vert\: x)\,q_{\pi}(x,u),\\
      &=v_{\pi}(x).
    \end{split}
  \end{equation}
  \normalsize\pause
...
\frame{\frametitle{MC-based control example: forest tree MDP (1)}
  \begin{figure}
    ...
    \caption{Different estimates of forest tree MDP ($\alpha=0.2, \gamma=0.8$) using MC control with \hl{$\varepsilon=0.20$} over the number of episodes. Mean is red and standard deviation is light blue, both calculated based on 2000 independent runs.}
...
\frame{\frametitle{Greedy in the limit with infinite exploration (GLIE)}
  \begin{defi}{Greedy in the limit with infinite exploration (GLIE)}{GLIE}
    ...
    \item If a state is visited infinitely often, then each action is chosen infinitely often:
      \begin{equation}
	\lim_{i\rightarrow\infty} \pi_i(u\:\vert\: x)=1 \quad \forall \, \left\{x\in\mathcal{X}, u\in\mathcal{U}\right\}\, .
      \end{equation}
    \item In the limit ($i\rightarrow \infty$) the learning policy is greedy with respect to the learned action value:
      \begin{equation}
	\lim_{i\rightarrow\infty} \pi_i(u\:\vert\: x)=\pi(x)= \argmax_{u} q(x, u) \quad \forall x\in\mathcal{X} \, .
      \end{equation}
    \end{itemize}
  \end{defi}
}
...
\frame{\frametitle{Off-policy learning background}
  ...
  \item Use two separated policies:
    \begin{itemize}
    \item \hl{Behavior policy} $b(u\:\vert\: x)$: explores in order to generate experience.
    \item \hl{Target policy} $\pi(u\:\vert\: x)$: learns from that experience to become the optimal policy.
    \end{itemize}\pause
  \item Use cases:
    \begin{itemize}
...
\frame{\frametitle{Off-policy prediction problem statement}
  \begin{block}{MC off-policy prediction problem statement}
    \begin{itemize}
    \item Estimate $v_\pi$ and/or $q_\pi$ while following $b(u\:\vert\: x)$.
  ...
  \item \hl{Coverage}: Every action taken under $\pi$ must be (at least occasionally) taken under $b$, too. Hence, it follows:
    \begin{equation}
      \pi(u\:\vert\: x) > 0 \Rightarrow b(u\:\vert\: x) > 0 \quad \forall \, \left\{x\in\mathcal{X}, u\in\mathcal{U}\right\} .
    \end{equation}\pause
...
\frame{\frametitle{Importance sampling}
  Starting in $X_{k}$ and following $\pi$, the probability of observing a particular trajectory on random variables $U_k, X_{k+1}, U_{k+1}, \ldots, X_{T}$ is
  \begin{equation}
    \begin{split}
      \Pb{U_k, X_{k+1}, U_{k+1}, \ldots , X_{T}\:\vert\:  X_{k}, \pi}&=\pi(U_k\:\vert\: X_{k})\,p(X_{k+1}\:\vert\: X_{k}, U_{k})\,\pi(U_{k+1}\:\vert\: X_{k+1})\cdots\\
      &=\prod_k^{T-1} \pi(U_k\:\vert\: X_{k}) \, p(X_{k+1}\:\vert\: X_{k}, U_{k}).
    \end{split}
  \end{equation}
  Where $p$ is the state-transition probability (cf.\ \defref{defi:Markov_decision_process}).\pause
  \begin{defi}{Importance sampling ratio}{import_sampl_ratio}
    ...
    \begin{equation}
      \rho_{k:T}=\frac{\prod_k^{T-1} \pi(U_k\:\vert\: X_{k}) \, p(X_{k+1}\:\vert\: X_{k}, U_{k})}{\prod_k^{T-1} b(U_k\:\vert\: X_{k}) \, p(X_{k+1}\:\vert\: X_{k}, U_{k})}=\frac{\prod_k^{T-1} \pi(U_k\:\vert\: X_{k})}{\prod_k^{T-1} b(U_k\:\vert\: X_{k})} .
...
\frame{\frametitle{Importance sampling for Monte Carlo prediction}
  \begin{defi}{State-value estimation via Monte Carlo importance sampling}{MC_ord_import_sampl}
    ...
    \begin{equation}
      \hat{v}_\pi(x_k)=\frac{\sum_{k\in\mathcal{T}(x_k)}\rho_{k:T(k)}g_k}{\lvert\mathcal{T}(x_k)\rvert }.
      \label{eq:OIS}
    \end{equation}
  \end{defi}
  Notation:
