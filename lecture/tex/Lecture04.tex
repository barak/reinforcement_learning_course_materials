...
\end{itemize}
  ...
  \onslide<1->
  \shortstack{
    \includegraphics[width=0.5\textwidth]{fig/lec04/Monte_Carlo_City}\\
    Monte Carlo port \\
  ...
  \onslide<5->
  \shortstack{
    \includegraphics[width=0.3\textwidth]{fig/lec04/gambling-in-monte-carlo}\\
    Bond. James Bond.\\
    (Warner Bros.)}
}
...
\onslide<7->{
\begin{itemize}
	\item \hl{If a given decision problem is non-stationary} or to reduce the memory demand storing the episode number $J$, using a forgetting factor $\alpha\in\left\{\Re|0<\alpha<1\right\}$ allows for continuous learning: 
\end{itemize}
\begin{equation}
...
\onslide<8->{
\begin{footnotebox}
	More information: S. Singh and  R. Sutton, \enquote{Reinforcement Learning with Replacing Eligibility Traces}, Machine Learning \textbf{22}:123--158, 1996
\end{footnotebox}}
}
...
\begin{figure}
	\subfloat{
		\includegraphics[height=2.5cm]{fig/lec04/Back_Up_DP}
	}
	\hspace{1cm}
	\subfloat{
		\includegraphics[height=2.5cm]{fig/lec04/Back_Up_MC}
	}
...
\vspace{0.5cm}
\begin{figure}		
	\includegraphics[height=0.5\textheight]{fig/lec04/Forest_Markov_Decision_Process_State_Value}
...
\begin{figure}		
	\includegraphics[height=0.65\textheight]{fig/lec04/Forest_Tree_MC_Value_Prediction}
...
\begin{itemize}
	\item<3-> Similarly to state values as in \algoref{algo:MC_state_prediction_first_visit}.
...
\begin{minipage}[c]{0.25\textwidth}
    \includegraphics[height=0.4\textheight]{fig/lec04/MC_GPI}
  \end{minipage}
...
\begin{itemize}
	...
	\begin{itemize}
		...
		\begin{equation}
			 \pi(u|x) > 0 \quad \forall\, \left\{x\in\mathcal{X}, u\in\mathcal{U}\right\} \hspace*{10em}
		\end{equation}
		\item<2-> Policies with this characteristic are called: \hl{soft}.
		\item<3-> Level of exploration can be tuned during learning.
	\end{itemize}
	\vspace{0.5cm}
	\item<4-> \hl{$\varepsilon$-greedy on-policy learning} 
		\begin{itemize}
		\item<4-> With probability $\varepsilon$ the agent's choice (i.e., the policy output) is replaced by a random action.
		\item<5-> Probability of all non-greedy actions: \begin{equation}\varepsilon/\lvert\mathcal{U}\rvert\,.\end{equation}
		\item<6-> Probability of the greedy action: \begin{equation}1-\varepsilon+\varepsilon/|\mathcal{U}|\,.\end{equation}
		\item<5-> Where $\lvert\mathcal{U}\rvert$ is the cardinality of the action space.
	\end{itemize}
\end{itemize}
  ...
  \begin{tikzpicture}[remember picture,overlay]
    ...
    {\rule{0em}{27ex}\includegraphics[width=0.3\textwidth]{fig/lec04/never-say-never-again-poster}\hspace*{3em}};
  \end{tikzpicture}
}
...
\SetKwInput{Init}{init}
\SetKwInput{Param}{parameter}
\Output{Optimal $\varepsilon$-greedy policy $\pi^*(u|x)$, \hspace{0.5cm}\textbf{parameter:} $\varepsilon\in\left\{\Re|0<\varepsilon \ll 1\right\}$}%\Param{}
...
\end{equation} 
\normalsize
\onslide<5->{In the inequality line, the second term is the weighted sum over action values given an $\varepsilon$-greedy policy. This weighted sum is bounded above by $\max_{u}q_{\pi}(x,u)$.}
}
...
\begin{figure}		
	\includegraphics[height=0.65\textheight]{fig/lec04/Forest_Tree_MC_Control_eps_02}
...
\begin{figure}		
	\includegraphics[height=0.65\textheight]{fig/lec04/Forest_Tree_MC_Control_eps_005}
...
Starting at $X_{k}$ and following $\pi$, the probability of observing a particular trajectory on random variables $U_k, X_{k+1}, U_{k+1}, \ldots, X_{T}$ is
\begin{equation}
\begin{split}
