...
\onslide<7->{
\begin{itemize}
	\item \hl{If a given decision problem is non-stationary} or to reduce the memory demand storing the episode number $J$, using a forgetting factor $\alpha\in\left\{\Re \vert 0<\alpha<1\right\}$ allows for continuous learning: 
\end{itemize}
\begin{equation}
...
\SetKwInput{Init}{init}
\SetKwInput{Param}{parameter}
\Output{Optimal $\varepsilon$-greedy policy $\pi^*(u|x)$, \hspace{0.5cm}\textbf{parameter:} $\varepsilon\in\left\{\Re \vert 0<\varepsilon\ll 1\right\}$}%\Param{}
...
\begin{equation}
	\begin{split}
		\onslide<2->{q_{\pi}(x,\pi'(x)) &= \sum_{u}\pi'(u|x)q_{\pi}(x,u)}\\
											...
											&\onslide<4->{\geq \frac{\varepsilon}{|\mathcal{U}|}\sum_{u}q_{\pi}(x,u) + (1-\varepsilon)\sum_{u}\frac{\pi(u \vert x)-\frac{\varepsilon}{|\mathcal{U}|}}{1-\varepsilon}q_{\pi}(x,u).}
\end{split}
\end{equation} 
\normalsize
...
\begin{equation}
	\begin{split}
		q_{\pi}(x,\pi'(x)) &\geq \frac{\varepsilon}{|\mathcal{U}|}\sum_{u}q_{\pi}(x,u) + (1-\varepsilon)\sum_{u}\frac{\pi(u \vert x)-\frac{\varepsilon}{|\mathcal{U}|}}{1-\varepsilon}q_{\pi}(x,u)\\
											&\onslide<2->{= \frac{\varepsilon}{|\mathcal{U}|}\sum_{u}\left(q_{\pi}(x,u) - q_{\pi}(x,u)\right) + \sum_{u}\pi(u \vert x)q_{\pi}(x,u)}\\
											&\onslide<3->{=\sum_{u}\pi(u \vert x)q_{\pi}(x,u)}\\
											&\onslide<4->{=v_{\pi}(x).}
	\end{split}
\end{equation} 
\normalsize
...
\begin{equation}
\begin{split}
	\Pb{U_k, X_{k+1}, U_{k+1}, \ldots , X_{T}| X_{k}, \pi}&=\pi(U_k \vert X_{k})p(X_{k+1}|X_{k}, U_{k})\pi(U_{k+1} \vert X_{k+1})\cdots\\
	&=\prod_k^{T-1} \pi(U_k \vert X_{k})p(X_{k+1}|X_{k}, U_{k}).
	\end{split}
\end{equation}
