...
\begin{itemize}
	...
	\begin{align*}
	v_\pi(x_k) &= \El{G_k \vert X_k=x_k}{\pi}\\
...
\begin{equation}
	v_{i+1}(x_k)	= \sum_{u_k\in\mathcal{U}}\bm{\pi}(u_k \vert x_k)\left(\mathcal{R}^u_x + \gamma\sum_{x_{k+1}\in\mathcal{X}}p_{xx'}^u v_{i}(x_{k+1})\right)\, .
\end{equation}\pause
...
\Init{$v_0(x)\, \forall \, x\in\mathcal{X}$ arbitrary except $v_0(x)=0$ if $x$ is terminal}
 \Repeat{$\Delta<\delta$}{
		...
		\For{$\forall \, x_k\in\mathcal{X}$}{
			...
			$\hat{v}(x_k)\leftarrow  \sum_{u_k\in\mathcal{U}}\pi(u_k \vert x_k)\left(\mathcal{R}^u_x + \gamma\sum_{x_{k+1}\in\mathcal{X}}p_{xx'}^u \hat{v}(x_{k+1})\right)$\;
...
\begin{itemize}
	...
	\item A proof for the policy improvement theorem was presented for deterministic policies, but transfer to stochastic policies $\pi(u_k \vert x_k)$ is straightforward.\pause
...
\begin{enumerate}
	\item $\pi_0=\pi(u_k =\text{c} \vert x_k) \quad \,\forall x_k\in\mathcal{X}$.\pause
	...
	\begin{split}
		\pi_1(x_{k})& = \argmax_{u_k\in\mathcal{U}} \E{R_{k+1} + \gamma 	v_{\pi_0}(X_{k+1})|X_{k}=x_{k},U_{k}=u_{k}}\\
										& = \left\{\pi(u_k =\text{w} \vert x_k=1), \pi(u_k =\text{c} \vert x_k=2), \pi(u_k =\text{c} \vert x_k=3)\right\}
	\end{split}
\end{equation*}\pause
...
\begin{enumerate}
	\item $\pi_0=\pi(u_k =\text{w} \vert x_k) \quad \,\forall x_k\in\mathcal{X}$.\pause
