...
\begin{itemize}
	...
	\begin{equation}
	\left\|v_{i}(x_{k}) - v_\pi\right\|_{\infty}\rightarrow 0 \quad \text{for} \quad i=1,2,3,\ldots
\end{equation}\pause
...
\end{equation}\pause
Or in Matrix form based on \eqref{eq:Bellman_MDP_linear}:
\begin{equation}
...
\end{equation}\pause
\begin{figure}		
	\includegraphics[height=0.25\textheight]{fig/lec03/Back_Up_Value_Policy_Evaluation}
...
Let's reuse the forest tree MDP example from \figref{fig:Forest_Markov_Decision_Process_State_Value} with the \textit{fifty-fifty policy}:
\begin{align*}
	\bm{\mathcal{P}}_{xx'}^{\pi} &=
        \begin{bmatrix}
          0 & \frac{1-\alpha}{2} & 0 & \frac{1+\alpha}{2} \\[1.1ex]
          0 & 0 &\frac{1-\alpha}{2} & \frac{1+\alpha}{2} \\[1.1ex]
          0 & 0 &\frac{1-\alpha}{2} & \frac{1+\alpha}{2} \\[1.1ex]
          0 & 0 & 0 & 1
        \end{bmatrix}
        &
        \bm{r}_{\mathcal{X}}^{\pi} &= \begin{bmatrix}0.5 \\ 1 \\ 2 \\ 0 \end{bmatrix}
        &
        \bm{v}_{i+1} &= \bm{r}+\gamma\bm{\mathcal{P}}_{xx'}^{\pi}\bm{v}_{i}
\end{align*}
\pause
\begin{table}
	\centering
...
\begin{itemize}
	...
	\item As shown in \tabref{tab:Forest_tree_policy_eval_in_place}, in-place updates converge faster, particularly for `early states'.
\end{itemize}
...
\begin{itemize}
	...
	\item A proof of the policy improvement theorem for deterministic policies was presented, but transfer to stochastic policies $\pi(u_k \vert x_k)$ is straightforward.\pause
...
\begin{figure}		
	\includegraphics[height=0.6\textheight]{fig/lec03/Forest_Markov_Decision_Process}
\end{figure}
\begin{itemize}
...
Assume $\alpha=0.2$ and $\gamma=0.8$ and initialize with \hl{`tree hater' policy}:\pause
\begin{enumerate}
	\item $\pi_0=\pi(u_k =\text{c}|x_k) \quad \,\forall x_k\in\mathcal{X}$.\pause
	...
	\begin{split}
		\pi_1(x_{k})& = \argmax_{u_k\in\mathcal{U}} \E{R_{k+1} + \gamma 	v_{\pi_0}(X_{k+1})|X_{k}=x_{k},U_{k}=u_{k}}\\
										& = \left\{\pi(u_k =\text{w}|x_k=1), \pi(u_k =\text{c}|x_k=2), \pi(u_k =\text{c}|x_k=3)\right\}
	\end{split}
\end{equation*}\pause
...
\begin{enumerate}
	\item $\pi_0=\pi(u_k =\text{w}|x_k) \quad \,\forall x_k\in\mathcal{X}$.\pause
...
\begin{table}
	\centering
		\begin{tabular}{cM{4.75cm}c}
			\toprule
			\textbf{Problem} & \textbf{Relevant Equations} & \textbf{Algorithm}\\
			\midrule
			prediction & Bellman expectation eq. &  policy evaluation\\[1.5ex]
			control & Bellman expectation eq. \& greedy policy improvement & policy iteration\\[2.5ex]
			control & Bellman optimality eq. & value iteration\\
			\bottomrule
		\end{tabular}	
		\caption{Overview of DP algorithms discussed}
...
\begin{figure}
	\subfloat{
		\includegraphics[height=4cm]{fig/lec03/GPI_01}
	}
	\hspace{1cm}
	\subfloat{
		\includegraphics[height=4cm]{fig/lec03/GPI_02}
	}
