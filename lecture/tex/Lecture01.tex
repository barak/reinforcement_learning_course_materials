...
% - Lecture01_intro_master.tex (Maynooth University version)
...
\begin{itemize}
	...
	\end{equation}
	\item Here, $\gamma\in\left\{\Re \vert 0 \leq \gamma \leq 1\right\}$ is the \hl{discount rate}.
\end{itemize}
}
...
\begin{equation}
	\bm{\pi}(\bm{U}_k \vert \bm{X}_k)=\Pb{\bm{U}_k | \bm{X}_k}\, .
\end{equation}
\pause
...
\begin{itemize}
	...
	\label{eq:state_value_lec01}
		v_\pi(\bm{x}_k) = \El{G_k \vert \bm{X}_k=\bm{x}_k}{\pi}=\El{\left. \sum_{i=0}^\infty \gamma^i R_{k+i+1}\right|\bm{x}_k}{\pi}.
	\end{equation}
	\pause
...
\end{itemize}
	\begin{equation}
		q_\pi(\bm{x}_k,\bm{u}_k) = \El{G_k \vert \bm{X}_k=\bm{x}_k, \bm{U}_k=\bm{u}_k}{\pi}=\El{\left.\sum_{i=0}^\infty \gamma^i R_{k+i+1}\right|\bm{x}_k, \bm{u}_k}{\pi}.
	\end{equation}\pause
\begin{itemize}
