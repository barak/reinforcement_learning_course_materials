...
\frame{\frametitle{Reward examples}
  ...
  \item Flipping a pancake:
    \begin{itemize}
    \item Positive reward: catching the $\unit{180}{^\circ}$ rotated pancake
    \item Negative reward: droping the pancake on the floor
    \end{itemize}
    \pause
  \item Stock trading:
    \begin{itemize}
...
\frame{\frametitle{The reward function hassle}
  \begin{itemize}
  \item ``Be careful what you wish for --- you might get it'' (proverb)
...
\frame{\frametitle{Task-dependent return definitions}
  ...
  \item The return should be \hl{discounted} to prevent infinite numbers:
    ...
    \end{equation}
    where $0\leq\gamma<1$ is the \hl{discount rate}.
  ...
  \begin{block}{\centerline{\LARGE WARNING}}
    \begin{Large}
      ...
      \item Does it have all the necessary information?
        ...
        The state is \emph{now} but might need to include past information.
      ...
      \item This issue was the root cause of the vast majority of failed class projects.
      \end{itemize}
    \end{Large}
  \end{block}
}
...
\frame{\frametitle{History and information state}
  ...
  \begin{defi}{Information state}{InfSate}
    ...
    \begin{equation}
      \Pb{\bm{X}_{k+1} \vert \bm{X}_k}=\Pb{\bm{X}_{k+1} \vert \bm{X}_0, \bm{X}_1,\ldots, \bm{X}_k}\, .
    \end{equation}
  \end{defi}
}
...
\frame{\frametitle{Policy}
  ...
  \item Stochastic policies: maps a probability of the action given a state:
    \begin{equation}
      \bm{\pi}(\bm{U}_k \vert \bm{X}_k)=\Pb{\bm{U}_k \vert \bm{X}_k}\, .
    \end{equation}
    \pause
...
\frame{\frametitle{Deterministic policy example}
  ...
  \begin{itemize}
    ...
    \onslide<3->\item	Reference value is part of the environment state: $\bm{x}_k = \begin{bmatrix}y_k & y^*_k\end{bmatrix}\T$.
      \onslide<4->\item Control output is the agent's action: $\bm{u}_k = \bm{\pi}(\bm{x}_k \vert K_\mathrm{p}, K_\mathrm{i}, K_\mathrm{d})$.
  \end{itemize}
...
\frame{\frametitle{Value functions}
  ...
  \item Assuming an MDP problem structure the state-value function is
    \begin{equation}
      ...
      v_\pi(\bm{x}_k) = \El{G_k \vert \bm{X}_k=\bm{x}_k}{\pi}=\El{\left. \sum_{i=0}^\infty \gamma^i R_{k+i+1}\right \vert \bm{x}_k}{\pi}.
    \end{equation}
    \pause
  ...
  \end{itemize}
  \begin{equation}
    q_\pi(\bm{x}_k,\bm{u}_k) = \El{G_k \vert \bm{X}_k=\bm{x}_k, \bm{U}_k=\bm{u}_k}{\pi}=\El{\left.\sum_{i=0}^\infty \gamma^i R_{k+i+1}\right \vert \bm{x}_k, \bm{u}_k}{\pi}.
  \end{equation}\pause
  \begin{itemize}
...
\frame{\frametitle{Model}
  ...
  \item That could be a state model $\bm{\mathcal{P}}$:
    \begin{equation}
      \bm{\mathcal{P}} = \Pb{\bm{X}_{k+1}=\bm{x}_{k+1} \vert \bm{X}_k=\bm{x}_k, \bm{U}_k=\bm{u}_k}\,.
    \end{equation}
    \pause
  \item Or a reward model $\mathcal{R}$:
    \begin{equation}
      \mathcal{R} = \Pb{R_{k+1}=r_{k+1} \vert \bm{X}_k=\bm{x}_k, \bm{U}_k=\bm{u}_k}\,.
    \end{equation}
    \pause
