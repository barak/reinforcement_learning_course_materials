...
\begin{figure}
	\subfloat{
		\includegraphics[height=3.6cm]{fig/lec03/GPI_01}
	}
	\hspace{1cm}
	\subfloat{
		\includegraphics[height=3.6cm]{fig/lec03/GPI_02}
	}
...
\begin{itemize}
	...
	\item Above, \hl{$\bm{w} \in \Re^\zeta$ is the parameter vector}. \pause
...
\begin{equation}
	\overline{\text{VE}}(\bm{w}) = \int_{\mathcal{X}}\mu(\bm{x})\left[v_{\pi}(\bm{x}) - \hat{v}(\tilde{\bm{x}}, \bm{w})\right]^2
\label{eq:VE}
\end{equation}
...
\vspace{0.5cm}
\begin{equation}
	\overline{\text{VE}}(\bm{w}) \approx J(\bm{w})= \sum_k \left[v_{\pi}(\bm{x}_k) - \hat{v}(\tilde{\bm{x}}_k, \bm{w})\right]^2 .
\label{eq:VE_J}
\end{equation}\pause
\vspace{0.25cm}
\begin{itemize}
...
\begin{figure}
	\includegraphics[width=4.5cm]{fig/lec09/Gradient_descent}
...
\end{itemize}
\begin{figure}
	\includegraphics[height=0.4\textheight]{fig/lec09/Picard_GD}
...
\pause
\begin{figure}
	\includegraphics[width=9.25cm]{fig/lec09/Simple_LinearRegression_Example}
...
\end{itemize}
\begin{equation}
	\bm{w}_{\text{Ridge}}^* = (\bm{\Xi}\T\bm{\Xi}+\epsilon\bm{I})^{-1}\bm{\Xi}\T\bm{y} .
	\label{eq:Ridge_LSTD}
\end{equation}
}
