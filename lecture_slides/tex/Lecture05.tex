...
\frame{\frametitle{SARSA example: forest tree MDP (3)}
  ...
  \begin{figure}
    ...
    \caption{\\SARSA-based control with \hl{adaptive $\alpha_{\text{SARSA}}=\frac{1}{\sqrt{j}}$} ($j=$episode) and $\varepsilon$-greedy policy with $\varepsilon=0.2$ of forest tree MDP over the number of episodes being evaluated (mean and standard deviation are calculated based on 2000 independent runs)}
