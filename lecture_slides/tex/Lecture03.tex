...
\frame{\frametitle{Iterative policy evaluation example: forest tree MDP}
  ...
  \begin{align*}
    ...
    \begin{bmatrix}
      0 & \frac{1-\alpha}{2} & 0 & \frac{1+\alpha}{2}  \\
      0 & 0 &\frac{1-\alpha}{2} & \frac{1+\alpha}{2} \\
      0 & 0 &\frac{1-\alpha}{2} & \frac{1+\alpha}{2} \\
      0 & 0 & 0 & 1
    \end{bmatrix}
    &
    \quad\bm{r}_{\mathcal{X}}^{\pi} &=
    \begin{bmatrix}
      0.5 \\ 1 \\ 2 \\ 0
    \end{bmatrix}
    &
...
\frame{\frametitle{In-place policy evaluation updates for forest tree MDP}
  ...
  \item As seen in \tabref{tab:Forest_tree_policy_eval_in_place}, in-place updates converge faster for the `early states' in particular.
  \end{itemize}
...
\frame{\frametitle{Summarizing DP algorithms}
  ...
  \begin{table}
    ...
    \begin{tabular}{cM{4.75cm}c}
      ...
      \bottomrule
    \end{tabular}
    \caption{Short overview addressing DP algorithms discussed}
...
\frame{\frametitle{Generalized policy iteration (GPI)}
  ...
  \begin{figure}
    ...
    \caption{Interpreting generalized policy iteration to switch back and forth between (arbitrary) evaluations and improvement steps (source: R. Sutton and G. Barto, \textit{Reinforcement Learning: An Introduction}, 2018, \href{https://creativecommons.org/licenses/by-nc-nd/2.0/}{CC BY-NC-ND 2.0})}
  \end{figure}
}
