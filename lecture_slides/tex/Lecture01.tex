...
\begin{itemize}
  \item \href{mailto:barak.pearlmutter@mu.ie}{\textbf{Barak A. Pearlmutter} $\langle \texttt{barak.pearlmutter@mu.ie} \rangle$}
    ...
    Professor, Dept of Computer Science. Has been working on theoretical neurobiology and neural networks since the second time they were cool. AI Doomer. Also into science fiction and functional programming languages and robust first-class automatic differentiation as an enabling technology for building adaptive systems.
...
\begin{block}{Course Conference (60\%)}
  \begin{itemize}
  \item Each student will do an RL-based project; development will be done in a git repo.
  \item Each student will submit a manuscript describing their project to the class conference.
  \item Abstracts (one ¶ text) will have an earlier deadline. Code will also be submitted.
  \item Manuscripts: rigid format (\LaTeX\ style\&template supplied), sections, page limit.
  \item The submission and review process will be double-blind.
  \item Each student will be a reviewer, and there will be reviews, responses, discussion, revisions, meta-reviews, etc. BP is the program chair.
  \item All submissions accepted as posters. Selected: oral spotlight, best paper award.
...
\begin{itemize}
		...
		\item Multi-step bootstrapping \pause
		\item Planning and model-based RL\pause  
		\item Function approximation and (deep) learning \pause
		\item Policy gradient methods \pause
		\item Contemporary (deep) RL algorithms \pause
